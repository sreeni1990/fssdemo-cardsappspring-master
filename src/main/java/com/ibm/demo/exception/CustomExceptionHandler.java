...
import com.ibm.demo.services.*;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.ibm.demo.exception.Error.SeverityEnum;
...
public class CustomExceptionHandler {
	...
	//#AutowireServices
	@Autowired
	private GetCustomerCardDetailsI getCustomerCardDetailsI;
	@Autowired
	private CreateCardTransactionI createCardTransactionI;
	@Autowired
	private GetCardAccountEnquiryI getCardAccountEnquiryI;
	@Autowired
	private UpdateDirectDebitsI updateDirectDebitsI;
	...
	private Object getErrorObject(String operationId,ErrorStatusCode code,Exception exception){
		...
		if(apiExceptionResponse != null) {
			try {
				switch(operationId){
			case "getCustomerCardDetails":
			object = getCustomerCardDetailsI.error(code.getCode(),apiExceptionResponse.getDataType(),exception);
			break;
			
			case "createCardTransaction":
			object = createCardTransactionI.error(code.getCode(),apiExceptionResponse.getDataType(),exception);
			break;
			
			case "getCardAccountEnquiry":
			object = getCardAccountEnquiryI.error(code.getCode(),apiExceptionResponse.getDataType(),exception);
			break;
			
			case "updateDirectDebits":
			object = updateDirectDebitsI.error(code.getCode(),apiExceptionResponse.getDataType(),exception);
			break;
